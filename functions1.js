...
function reserve(type, machid, start_date, resid, scheduleid, is_blackout, read_only, pending, starttime, endtime) {  
	...
	//	window.location= "http://encyclopedia.gwu.edu/srrs/schedule1.php";
		nurl = "reserve1.php?type=" + type + "&machid=" + machid + "&start_date=" + start_date + "&resid=" + resid + '&scheduleid=' + scheduleid + "&is_blackout=" + is_blackout + "&read_only=" + read_only + "&pending=" + pending + "&starttime=" + starttime + "&endtime=" + endtime;    
		var resWindow = window.open(nurl,"reserve","width=" + w + ",height=" + h + ",scrollbars,resizable=no,status=no");     
		resWindow.focus();
	
		 var evt = resWindow.document.createEvent("MouseEvents");
		 evt.initMouseEvent("click", true, true, window,0, 0, 0, 0, 0, false, false, false, false, 0, null);
		 var cb = resWindow.document.getElementById("btnSubmit");
		 resWindow.document.write(cb.name);
		 resWindow.document.write("Raza");
